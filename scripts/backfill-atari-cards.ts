/**
 * One-time migration: backfill atariCards on existing oripa-posts.
 *
 * - Scans the tweets table (up to LIMIT tweets)
 * - Re-analyzes each tweet with the current AI schema via Bedrock
 * - For each tier where atariCards is detected, updates the
 ...
 *     pnpm --filter @oripa-now/scripts exec tsx backfill-atari-cards.ts
 *
 * Options (env vars):
...
async function updateAtariCards(postId: string, cards: string[]): Promise<void> {
  await docClient.send(
    new UpdateCommand({
      TableName: TABLE_NAMES.oripaPosts,
      Key: { postId },
      UpdateExpression: 'SET atariCards = :cards, updatedAt = :updatedAt',
...
async function main() {
  console.log(`=== Backfill atariCards ===`);
  ...
  for (const tweet of tweets) {
    try {
      ...
      for (let i = 0; i < result.items.length; i++) {
        ...
        if (!tier.atariCards || tier.atariCards.length === 0) continue;

        const postId = i === 0 ? tweet.tweetId : `${tweet.tweetId}-${i}`;
        await updateAtariCards(postId, tier.atariCards);
        console.log(`✓ ${postId} → [${tier.atariCards.join(', ')}]`);
        updated++;
        atariFound = true;
      }

      if (!atariFound) skipped++;
    } catch (err) {
  ...
  console.log(`Skipped (no atari): ${skipped}`);
